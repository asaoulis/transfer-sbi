...
def train_model(config, pretrain=False):
    train_x, train_y, test_data = get_dataset(config.dataset_name, config.dataset_size, config.scaling_dataset)
    ...
    logger = wandb.init(
        ...
        name=f"{'pretrain' if pretrain else 'finetune'}_{config.dataset_name}_{config.scheduler_type}",
        reinit=True
    )

    fit_model(model, config.epochs, logger, train_loader, val_loader)
