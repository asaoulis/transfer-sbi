...
def prepare_data_and_model(config, data_parameters = None):
    ...
    model = NDELightningModule(
            ...
            checkpoint_path=config.checkpoint_path, freeze_CNN=config.freeze_cnn,  scheduler_kwargs={**config.scheduler_kwargs, **{'warmup': min(config.dataset_size // 4, 1000)}}
        )
...
def prepare_data_parameters(config):
    if config.dataset_name == "log_normal":
        ...
        train_x, train_y, valid_x, valid_y, test_data, scalers = get_dataset(
            config.dataset_name, config.dataset_suite, config.dataset_size, config.scaling_dataset, config.data_seed, config.unpaired
        )
